...
console.log("Emscripten version: D:/Unreal/UE_4.23/Engine/Extras/ThirdPartyNotUE/emsdk/emscripten/1.38.31");
console.log("Emscripten configuration: D:/Unreal/UE_4.23/Engine/Intermediate/Build/HTML5/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_html5project', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202206092236, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../html5project/html5project.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('html5project.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('html5project.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('html5project.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('html5project.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('html5project.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
